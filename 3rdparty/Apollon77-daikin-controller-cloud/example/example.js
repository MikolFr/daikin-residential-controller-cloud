...
async function main() {
    ...
    const options = {
        ...
        proxyDataDir: './'            // Directory to store certificates and other proxy relevant data to
    };

    let tokenSet;

    // Load Tokens if they already exist on disk
    const tokenFile = path.join('./', 'tokenset.json');
...
*/
      console.log(`Token not saved, need to generate before.`);
      return;
    }
    ...
    if (devices && devices.length) {
        for (let dev of devices) {
            ...
            console.log('    all data: ' + JSON.stringify(dev.getData()));
            console.log('    name: ' + dev.getData('climateControl', 'name').value);
            ...
            //await dev.setData('climateControl', 'onOffMode', 'off');
